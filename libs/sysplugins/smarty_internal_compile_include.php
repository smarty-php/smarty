...
class Smarty_Internal_Compile_Include extends Smarty_Internal_CompileBase
{
    ...
    public function compile($args, Smarty_Internal_TemplateCompiler $compiler, $parameter = null)
    {
        ...
        if (preg_match('/^([\'"])(([A-Za-z0-9_\-]{2,})[:])?(([^$()]+)|(.+))\1$/', $source_resource, $match)) {
            ...
            $handler = $compiler->smarty->_getExtension('_source')
        ...
        // set flag to cache sub-template object when called within loop or template name is variable.
        ...
        * the sub-template must not be included into the common cache file and is treated like
        ...
        */
        ...
        // if sub-template will be called in nocache mode do not merge
     ...
     * @return bool
     */
    public function compileInlineTemplate(Smarty_Internal_TemplateCompiler $compiler, $fullResourceName, $_caching,
                                          $hashResourceName, $t_hash, $c_id)
    {
        ...
        if (!($tpl->source->handler->uncompiled) && $tpl->source->exists) {
            $compiler->parent_compiler->mergedSubTemplatesData[ $hashResourceName ][ $t_hash ][ 'uid' ] =
                $tpl->source->uid;
            if (isset($compiler->template->_extensions[ '_inheritance' ])) {
                $tpl->_extensions[ '_inheritance' ] = clone $compiler->template->_extensions[ '_inheritance' ];
            }
            $tpl->loadCompiled();
            /* @var Smarty_Internal_TemplateCompiler $comp */
            $comp = $tpl->_getExtension('_compile')
                        ->loadCompiler($tpl);
            // save unique function name
            $compiler->parent_compiler->mergedSubTemplatesData[ $hashResourceName ][ $t_hash ][ 'func' ] =
            $tpl->compiled->renderCallback = 'content_' . str_replace(array('.', ','), '_', uniqid('', true));
            ...
            $compiled_code .= $comp->compileTemplateSource($tpl, null, $compiler->parent_compiler);
            $compiled_code .= "}\n";
            ...
            $compiler->parent_compiler->mergedSubTemplatesCode[ $tpl->compiled->renderCallback ] = $compiled_code;
            return true;
        } else {
            return false;
        }
